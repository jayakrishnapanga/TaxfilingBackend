...
//     // Find files associated with the given userId and sort by uploadDate in descending order
//     const filesCursor = bucket.find({ 'metadata.userId': userId }).sort({ uploadDate: -1 });
...
//     // Group the files based on submissionId
//     const groupedFiles = groupFilesBySubmissionId(files);

//     // res.render('files', { files });
//     console.log("this is from grouped files")
//     console.log(groupedFiles)
//     res.json({ groupedFiles }); // Render the 'files.ejs' template and pass the files data
...
// this done sort by uploaded time 
// app.get('/files/:userId', async (req, res) => {
//   try {
//     const userId = req.params.userId;
...
//     // Sort the files by uploadDate in descending order
...
// });
 ...
 // this code works fine sending 10 files only
// app.get('/files/:userId', async (req, res) => {
//   try {
//     const userId = req.params.userId;
...
//     // Calculate the total number of pages based on the number of submissions per page
//     const totalPages = Math.ceil(Object.keys(groupedFiles).length / submissionsPerPage);

//     // Determine the starting index and ending index of the submissions to be included on the current page
//     const startIndex = (page - 1) * submissionsPerPage;
//     const endIndex = startIndex + submissionsPerPage;

//     // Use the slice method to extract the submissions for the current page
//     const currentSubmissions = Object.values(groupedFiles).slice(startIndex, endIndex);

//     // res.render('files', { files });
//     console.log("this is from grouped files")
//     console.log(groupedFiles)
//     res.json({ currentSubmissions, totalPages }); // Return the current page's submissions and total pages
...
app.get('/files/:userId', async (req, res) => {
  try {
    ...
    // Create an object to store grouped files
    const groupedFiles = {};

    // Iterate over the cursor to get the files and group them by submissionId
    await filesCursor.forEach(file => {
      const { submissionId } = file.metadata;
    ...
    const totalSubmissions = Object.keys(groupedFiles).length;

    // Calculate the total number of pages for grouped files
    ...
    // Create an array to store the retrieved files for the current page
    const currentSubmissions = Object.values(groupedFiles).slice(startIndex, endIndex);
    console.log(currentSubmissions)

    res.json({ currentSubmissions, totalPages }); // Send the currentSubmissions and totalPages in the response
